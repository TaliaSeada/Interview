...
def main():
    st.set_page_config(page_title="Titanic CV Report", layout="wide")
    ...
    with st.sidebar:
        ...
        cv_results_path = st.text_input("CV results", "model/cv_results.csv")
        cv_predictions_path = st.text_input("CV predictions", "model/cv_predictions.csv")
        metadata_path = st.text_input("CV metadata", "model/cv_metadata.json")
        threshold = st.slider("Decision threshold", 0.0, 1.0, 0.5, 0.01)

    paths = {
        "dataset": Path(data_path),
        "cv results": Path(cv_results_path),
        "cv predictions": Path(cv_predictions_path),
        "cv metadata": Path(metadata_path),
    }
